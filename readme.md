...
This repo contains the code for the submission "A Simplified Model for Linear Mode Coupling in Multimode Fibers with Experimental Assessment" to the 2026 European Conference on Optical Communications. A brief description of our method is given below. For more details refer to the paper.

Two multisectional models with the following transfer matrix for the $i$-th section are considered:

$$ \mathbf{K}_i \mathbf{R} $$

where $\mathbf{R}$ is a block diagonal matrix with random unitary blocks uniformly distributed over the relative $\mathrm{U}(N_A)$ group, where $N_A$ is the size of the $A$-th mode group, and

$$ \mathbf{K}_i = e^{j \tilde{\mathbf{B}_0} + \tilde{\Delta \beta} g(\mathrm{XT}_t, G) (\mathbf{P}_i-\mathbf{P}_i^\dagger)}
$$ 
...
is the diagonal matrix containing the product between the section length $L$ and the propagation constants $\beta_0^{(n)}$ ($n$ is the mode index) for a graded-index multimode fiber (GIMMF) which are assumed to be constant within a mode group and to differ by $\Delta \beta$ from the neighbouring groups (the term $\tilde{\Delta \beta} = \Delta \beta  L$ can also be regarded as an optimization parameter or kept fixed to a certain value, e.g., $10^4$), and 

$$ \mathbf{P}_i $$

is a matrix whose elements are independent complex Gaussian random variables $P_{ab} \sim \mathcal{CN}(0, h_{A-B}^2)$, where $A$ is the group index of mode $a$ and $B$ is the group index of mode $b$. 

In the paper, two choices for $h_{A-B}$ are considered:

1) $h_{A-B} = 1$, i.e., same variance for all coupling coefficients. We call this the "uniform" choice. For $\mathrm{XT}_t \le -5\,\mathrm{dB}$, an approximate expression for $g$ is

 $$ g(\mathrm{XT}_t, G) \approx \sqrt{\frac{\mathrm{XT}_t - 0.91 + 0.31G}{2M}} $$ 
 
   where $\mathrm{XT}_t$ is the target crosstalk and $M$ is the total number of polarization modes; 
